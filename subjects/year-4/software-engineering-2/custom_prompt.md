...
**إلزامي في كل قسم فيه `#### ⚠️ أخطاء شائعة`:** استخدم كتلة **compare** بالشكل أدناه حرفياً — البارسر يحوّلها لبطاقة مقارنة على الموقع.
...
**قواعس إلزامية:**
- الرأس يجب يكون `####` (مستوى heading 4) — **ممنوع** `**الفهم الخاطئ ❌:**` أو `**الفهم الصحيح ✅:**`
- ممنوع bullets `- ❌` / `- ✅` لهذه الكتلة (تلك فقط لتعليل MCQ)
- ممنوع `**❌ الخطأ:**` / `✅ **الصحيح:**` كنص عادي
- يمكن تكرار الزوج أكثر من مرة إن وُجدت عدة أخطاء؛ افصل بـ `---` عند الحاجة
